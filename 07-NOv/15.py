...
# Apply an average filter to the noisy image
...
# Apply the average filter to the noisy image
filtered_image = custom_average_filter(img, kernel)

# Perform opening on the filtered image
opened_image = custom_opening(filtered_image, kernel)

# Perform closing on the noisy image
closed_image = custom_closing(img, kernel)

# Perform erosion on the filtered image
eroded_image = custom_erosion(filtered_image, kernel)

# Perform dilation on the eroded image
dilated_image = custom_dilation(eroded_image, kernel)

# Perform boundary extraction
boundary_image = custom_boundary_extraction(filtered_image, eroded_image)

# Display the original, noisy, filtered, opened, closed, eroded, dilated, and boundary images
